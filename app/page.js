        ...
        socket.onmessage = (event) => {
          try {
            ...
            if (data.imu) {
              ...
              // Smooth transitions
              setPitch(prev => prev + (p - prev) * 0.3);
              setRoll(prev => prev + (r - prev) * 0.3);
              setYaw(prev => prev + (y - prev) * 0.3);
              setHeading(prev => prev + (h - prev) * 0.3);
            }
            ...
            // Handle SET command response
            if (data.status === 'success') {
              console.log('✓ SET command successful:', data.message);
            }
...
Status: ZERO REFERENCE SET
`;
    const blob = new Blob([txt], { type: 'text/plain' });
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `Artillery_SET_${Date.now()}.txt`;
    a.click();
  ...
  const handleSet = () => {
    ...
    try {
      ...
      console.log('SET command sent to server');
      
      // Save log file
      saveToFile();
      
      // Show confirmation
      alert('✓ SET Confirmed\n\nCurrent position is now the 0° reference.\nIMU values will reset to zero.');
      
    } catch (error) {
      console.error('Error sending SET command:', error);
      alert('❌ Failed to send SET command');
    }
  };
    ...
    <div className="container">
      ...
      <header className="header">
        ...
        <button 
          onClick={handleSet} 
          disabled={!connected} 
          className={`set-btn ${connected ? '' : 'disabled'}`}
        >
          SET
        </button>
      </header>
